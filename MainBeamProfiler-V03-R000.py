...
title = "X-Y-Z Scan - All values in mm and seconds V002-R001"
...
class MyFrame(wx.Frame):
    def __init__(self, parent, title, xSize, ySize, panelWidth):
        ...
        def moveRight(event):
            #global s
            self.updateStatus('moving right',0)
            grblPort = bp.grblPort
            stepXY = bp.stepXY
            try:
                if bp.s is None:
        ...
        def moveBack(event):
            ...
            bp.travelZ = travelZ
            self.horZPos.SetLabel(label = "Zpos:_" + str(bp.travelZ))
        ...
        # create a first menu
        firstMenu = wx.Menu()
        saveMenuItem = firstMenu.Append(100, "Save","save data in file")
        # create a 2nd menu
        secondMenu = wx.Menu()
        ...
        #adding Z-position - 23 jan 20
        self.horZPos = wx.StaticText(self.grblPanel, label = "Zpos:_" + str(bp.dataInput[6]))
        self.horZPos.SetFont(font2)
        horZposSzr = wx.BoxSizer(wx.VERTICAL)
        horZposSzr.Add(self.horZPos, 0, wx.ALIGN_CENTER)
        ...
        s40 = wx.StaticBoxSizer(self.billboardBox, wx.VERTICAL)
        ...
        s40.Add(self.billboardBtn, 0, wx.EXPAND, 0)
        ...
        #s51.Add(self.saveBtn, 0, wx.ALIGN_CENTER_HORIZONTAL, 0)

        #putting START btn + Save + ScanChoice together
    ...
    def updateStatus(mf,f):
        msg = 'scan' + str(bp.scan) + '  X' + str(bp.Xtravel) + '  Y' + str(bp.Ytravel) \
    ...
    def receiveBkg(self, bkg):
        ...
        #bp.centroid = dp.calc_centroid(bp.imgData, bp.dataInput)
        #bp.secMoment = dp.calc_secmom(bp.imgData, bp.dataInput, bp.centroid)
        #print "centroids: ", bp.centroid[0],"...",bp.centroid[1]
        #print "secMoms: ", bp.secMoment[0],"...",bp.secMoment[1]
...
class showImage(wx.Frame):
    def __init__(self, title = "BP-image"):
...
class DisplayResults(wx.Frame):
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, title='Results: X0 & FW4S')
...
def graph():  #plot is the panel on which the PlotCanvas is defined
    ...
    if scan == "XY":
        if stp < nstepX + 2:
            ...
            #print "graph final...nstepX: ", nstepX,"..stp..: ",stp
            for n in range(nstepX + 1):
                #print "loop for n..",n,"value: ",bp.imgData[n]
                dv = bp.imgData[n][0]
                if dv > maxY:
                    maxY = dv * 1.1
                yRange = (bp.minVal, maxY )
            ...
            #print "graph -- -- ",plotTitle
            
    #defining plot for 2Dscan
    if scan == "2D":
        plotTitle = "Plot 2D X-axis 0/" + str(bp.nstepY + 1)
        if stp < nstepX + 1:
            line = wx.lib.plot.PolyLine(bp.data, colour='red', width=2)
            pg = wx.lib.plot.PlotGraphics([line], plotTitle, bp.xlabel, bp.ylabel)
        elif stp < nsteptot + 1:
            ...
            plotTitle = "Plot 2D X-axis " + str(ystp) + "/" + str(bp.nstepY + 1)
            line = wx.lib.plot.PolyLine(bp.data, colour='blue', width=2)
            pg = wx.lib.plot.PlotGraphics([line], plotTitle, bp.xlabel, bp.ylabel)
...
def startScan(self):
    ...
    try:
        ...
        statusbar.SetStatusText('invalid data... end scan', 1)
        endScan()
...
def takeData(self):
    ...
    if bp.scanning:
        ...
        mf.statusbar.SetStatusText('scanning...' + str(bp.scan), 1)
        ...
        if bp.scan == "XY":
            ...
            if bp.stp < bp.nstepX :
                ...
                #print "traveled X and Y..",gcodeTravelNext
                bp.stp += 1
                ...
                #print "Y of XYScan ...stp...",bp.stp,"...ystp...", ystp,"...of..",bp.nstepY
                dataPoint = ( ystp * bp.stepXY, datVal )
                bp.data.append(dataPoint)
                bp.imgData[ystp].append(datVal)
                bp.s.write(str(bp.gcodeTravelY))
