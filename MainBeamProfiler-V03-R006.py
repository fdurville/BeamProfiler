...
class MyFrame(wx.Frame):
    def __init__(self, parent, title, xSize, ySize, panelWidth):
        ...
        infoMenuItem = thirdMenu.Append(301, "About","info on this")
        # add menu to menubar
    ...
    #open a new frame to graph BP data (not used yet - FD-27nov29)
...
#not tested yet in this  - FD-27nov20
#this plots one set of BP data
#needs to be modified to plot all sets vs Z
#        adding rescaling  -  FD-19nov20
class GraphBPData(wx.Frame):
    def __init__(self, window_parent ):
        ...
        self.xmax = 5
        self.xRange = (0,self.xmax)
        self.yRange = (-10,self.ymax)
        self.autoScale = True

        self.plotTitleNS = "BPZ Plot  (Not Saved)"
        ...
        lin = wx.lib.plot.PolyLine(self.dummy, colour='red', width=2)
        line = [lin]

        if len(rd) > 3:
            for dd in range(len(rd[3]) - 1):
                datval = float( rd[3][dd] )
                print ("datval: "), datval, "dd: ", dd
                #if datval > self.ymax:
                    #self.ymax = 1.2 * datval
                    #self.yRange = (0,self.ymax)
                dt = dd
            ...
            line1 = wx.lib.plot.PolyLine(self.tdat, colour='red', width=2)
            line = line + [line1]
            
        #line2 = wx.lib.plot.PolyLine(bp1, colour='blue', width=2)
        #line = line + [line2]
    ...
    def OnScale(self, event):
        print ('Selected Sacling Graph Menu')
        ...
        if dlg.ShowModal() == wx.ID_OK :
            ...
            s3=''
            s4=''
            print ('init vals:'),v1,v2,v3,v4
            
            #this parsing now works with any float - FD-201120
            for c in newScale:
                if ci == 3 and c != ',':
                    s4 = s4 + c
                elif ci == 3 and c == ',':
